/*************************************************************************************
  ...
  2) Connect Arduino D2 to PH Meter Board T2 (DS18B20)  //REMOVER
...
#define ONE_WIRE_BUS 2                         // DS18B20 connect to Pin 2 //REMOVER
...
int CalSelect = 0;                             // 0 = PH1 Calibration Select , 1 = PH7 Calibration Select
...
double Ph1Buffer = 1.02;                       // For PH1 buffer solution's PH value , 1 or 1.02  //VERIFICAR VALORES CORRETOS

double Ph7Reading = 331;                       // PH7 Buffer Solution Reading.
double Ph1Reading = 522;                       // PH1 Buffer Solution Reading. //VERIFICAR VALORES CORRETOS
...
int reading(){                                   // Reading LM35 and PH Data
  ...
  //TempValue = (double) (TempValue-32)/1.8;  //Converte para Celsius
...
void setup(){
   ...
   PhRatio = (Ph1Reading - Ph7Reading) / (Ph7Buffer - Ph1Buffer);                 // Calculate Ph Ratio 
...
if ( RxCmd[1] == 1 ){
  ...
  switch (RxCmd[2]) {
    case 1:{                                                                                               //REMOVER
      Serial.print("Command 1 Received ");              // Enquiry Water Temperature (DS18B20)            //REMOVER
      Serial.println(sensors.getTempCByIndex(0),2);       // Return DS18B20 Data                           //REMOVER
      break;
    }
    case 2:{
...
if (CurrentMode == 0)                        // Normal Display Mode
{
  ...
  // Display 18B20 Temperature                                                                   //REMOVER
  //lcd.setCursor(6,1);                       // move cursor to second line "1" and 6 spaces over    //REMOVER
  //sensors.requestTemperatures();            // Read DS18B20 data                                  //REMOVER
  //lcd.print(sensors.getTempCByIndex(0));    // Display DS18B20 Data                               //REMOVER
...
if (CurrentMode == 4){            // PH1 Calibration Mode
  reading();
  lcd.setCursor(0,0);
  lcd.print("PH1 Cal. Mode");
  lcd.setCursor(0,1);
  lcd.print("C:");
  lcd.setCursor(2,1);
